...
<script>
        function translateToYusufish() {
            const words = {
                ...
                "5": "LARIARIIRAIRA",
                "6": "MDRRWERWER",
                "7": "MDRRRJARIARIARIJRAI",
                "8": "LIARIAAA JARIJARIJARIJARIARIJAAHAH",
                "9": "MDRRRR",
                "0": "KARIARIAR",
                ".": "NAGIAIDFS",
                ",": "ALRIARAIR",
            ...
            // Replace each word in the input text with its Yusufish equivalent
            let translatedText = text.split(' ').map(word => {
                return word.split('').map(char => words[char.toLowerCase()] || char).join('');
            }).join(' ');
    ...
    let insults = [
        ...
        "if cringe had a face, it would be him 😬",
        "tries to roast others but only burns himself 🔥",
        "thinks he's Sigma but acts like a Discord kitten 🐱"
    ];
